				...
				<view class="mt30 top_box">
					...
					<view class="mt34  ">
						...
						<view class="items ">
							...
							<view :style="topItemStyle">{{pageData.amount +  " "+currency}}</view>
						</view>
...
<!-- 						<qrcode-vue :value="pageData.qrcode"  :size="163" level="H" /> -->
							<image :src="pageData.qrcode"  mode="widthFix" v-if="pageData.qrcode" style="width: 320rpx;height:320rpx">
							</image>
							<image src="/static/themeNum1/icon/btc.png" mode="widthFix" v-else style="width: 320rpx;height:320rpx">
							</image>
						</view>
					</view>
					<view class="mt35 f26">
					...
					<view class="mt23 f22 addCopyEl " @click="copyHandle(addresData)">
						...
						<view class="copy" @click="copyHandle(addresData)">copy</view>
					</view>
				...
				<view class="btns mt22 center" :style="noStyle" style="box-shadow: none;background-color: #1D1D1D;"
					@click="cancleOrder">
					<view>
						{{t('wr.r_r9')}}
					</view>
			...
			<div class="wrapper">
				<div class="content ">
					...
					<view class="between">
						...
						<view @click="confirmHandle" style="color: #fff;background: #DE3824;"> {{t('all.a_c1')}}
						</view>
					</view>
				</div>
			</div>
			...
			if (!uploadTxid.value) {
				uni.showModal({
					...
					success: e => {
						if (e.confirm) {
							uni.navigateTo({
								url: "./offlineTxid?order=" + pageData.value.order_no+'&btc=1'
							})
						} else {
