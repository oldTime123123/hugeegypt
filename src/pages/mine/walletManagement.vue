<template>
	<view class="walletManagement">
		...
		<view class="pdlr35 pt53">
			...
			<view class="f60 mt60 text_bold" style="color: #fff">{{t('ss1.s_s8')}}</view>
	...
	const getData = () => {
		request({
			...
			if (withdraw_type.includes(1)) {
				settingList.value.find(item => item.url == '../setting/set2').flag = true
				// settingList.value[2].flag = true
			}
			
			
			if (withdraw_type.includes(3)) {
				settingList.value.find(item => item.url == '../setting/btcBind').flag = true
			}
			if (withdraw_type.includes(2)) {
				settingList.value.find(item => item.url == '../setting/set1').flag = true
			}
		})
	}
	const confirmHandle = () => {
