			...
			<view class="mt55">
				...
				<view class="topBox  flex-col" style="border-radius: 30rpx;">
					...
					<view class="topBoxItem">
						...
						<view class="mt35 f55" style="color: #000;" v-if="tabClick == 2"> {{ pageData.balance_max }} {{ currency }}</view>
						<view class="mt35 f55" style="color: #000;" v-if="tabClick == 1"> {{ pageData1.balance_max }} {{ currency }}</view>
				...
				<view v-else>
					...
					<view v-if="pageData.input_type == 2 && inputNum" style="color: #fff;display: flex;">
						<view class="short">≈USDT:</view>
						<text style="margin-left: 10px;">{{ (inputNum *
							(pageData.u_rate ? pageData.u_rate : 1)).toFixed(2)}}</text>
					</view>
...
const getData = (tabClick) => {
	request({
		url: `/finance/bank/withdraw/index?balance_type=2`,
		methods: 'get',
	}).then(res => {
		pageData.value = res
	...
	request({
		url: `/finance/bank/withdraw/index?balance_type=1`,
		methods: 'get',
	}).then(res => {
		pageData1.value = res
