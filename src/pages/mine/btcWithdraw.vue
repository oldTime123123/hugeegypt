			...
			<view class="mt55">
				...
				<view class="mt38">
					...
					<view class="mt34 otpEl">
						...
						<view class="otp" v-if="!pageData.user_link" @click="changePage('../setting/btcBind')">
							{{t('wr.w_u5')}}
						</view>
					</view>
				</view>
...
<script setup>
	...
	const getData = (tabClick) => {
		request({
			url: `finance/btc/withdraw/index?balance_type=${tabClick?tabClick:1}`,
			methods: 'get',
		}).then(res => {
			pageData.value = res
	...
	const submitHandle1 = () => {
		...
		request({
			url: 'finance/btc/withdraw/submit',
			methods: 'post',
			data: data
		}).then(res => {
				// getData();
