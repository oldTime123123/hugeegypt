	...
	<view :style="store.$state.imgObj.loginBg">
		<view class="pdlr48 pt33">
			...
			<view class="mt20 bBox" >
				...
				<view class="mt20 f70">{{pageData.balance}} {{currency}}</view>
				<view class="mt26 boxItemBox ">
					<view class="boxItem" v-for="item in boxItem">
						<view class="title mgtb10  f20">{{item.name}}</view>
						<view class="f24  mt20 secondClo" :style="{color:store.$state.thirdColor}">{{item.value}}</view>
					</view>
				</view>
			</view>
			...
			<!-- withdraw -->
			<view class="between ">
				<view class="bItem color0" :style="{background:store.$state.contentColor}" @click="goRecharge">
					<view class="mglr27">
						{{t('wr.r_r1')}}
					</view>
				</view>
				<view class="bItem" style="background: #fff;" :style="{color:store.$state.thirdColor}"
			...
			<view class="secondClo mt64 f40" :style="{color:store.$state.thirdColor}">
				{{t('mine.m_m12')}}
			</view>
			...
			<view class="mt60 myEl">
				<view class="myItem" v-for="(item,index) in myList" @click="changePage(item.url,item.openLink,item)"
					...
					<image :src="item.img" mode="widthFix" style="width: 75rpx;height: 75rpx;"></image>
					<view class="mglr35  textHidden " style="flex: 1;">{{item.name}}</view>
				</view>
			</view>
		</view>
...
<script setup>
	...
	const getData = () => {
		request({
			...
			pageData.value = res
			boxItem.value[0].value = res.total_commission
			boxItem.value[1].value = res.lixibao_balance
			boxItem.value[2].value = res.total_lixibao
		})
		// 获取客服状态
		request({
		...
		request({
			...
			if (res.download_status == 1) {
				myList.value[5].showFlag = true
				myList.value[5].url = res.url
			}
		})
		// 获取app状态
		request({
...
<style lang="scss">
	...
	.myEl {
		...
		.myItem {
			padding: 17rpx 59rpx 17rpx 10rpx;
			display: flex;
			align-items: center;
			// width: 100%;
			margin-top: 40rpx;
