   ...
   return new Promise(async (resolve, reject) => {
      ...
      function next() {
         uni.request({
            ...
            success(res: any) {
               if (res.data.version && !window.location.hash.includes('login')) {
                  let localVersion = uni.getStorageSync('localVersion');
                  if (!localVersion) {
                     localStorage.setItem('localVersion', res.data.version);
                     window.location.reload();
                  } else if (localVersion !== res.data.version) {
                     localStorage.setItem('localVersion', res.data.version);
                     window.location.reload();
                  }
               }
